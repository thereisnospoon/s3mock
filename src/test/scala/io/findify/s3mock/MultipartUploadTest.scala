...
class MultipartUploadTest extends S3MockTest {
  ...
  "s3" should "upload multipart files" in {
    ...
    val response1 = Await.result(
      http.singleRequest(
        HttpRequest(method = HttpMethods.POST,
                    uri = "http://127.0.0.1:8001/getput/foo2?uploads")),
      10.minutes)
    val data = Await.result(response1.entity.dataBytes
                              .fold(ByteString(""))(_ ++ _)
                              .runWith(Sink.head),
                            10.seconds)
    val uploadId =
    ...
    val response2 = Await.result(
      http.singleRequest(HttpRequest(
        ...
        uri =
          s"http://127.0.0.1:8001/getput/foo2?partNumber=1&uploadId=$uploadId",
        entity = "foo")),
      10.minutes
    )
    ...
    val response3 = Await.result(
      http.singleRequest(HttpRequest(
        method = HttpMethods.POST,
        uri =
          s"http://127.0.0.1:8001/getput/foo2?partNumber=2&uploadId=$uploadId",
        entity = "boo")),
      10.minutes
    )
    ...
    val response4 = Await.result(
      http.singleRequest(
        HttpRequest(method = HttpMethods.POST,
                    uri =
                      s"http://127.0.0.1:8001/getput/foo2?uploadId=$uploadId",
                    entity = commit)),
      10.minutes)
  ...
  it should "work with large blobs" in {
    ...
    val result = s3.completeMultipartUpload(
      new CompleteMultipartUploadRequest("getput",
                                         "fooLarge",
                                         init.getUploadId,
                                         blobs.map(_._2).asJava))
    result.getKey shouldBe "fooLarge"
    DigestUtils.md5Hex(s3.getObject("getput", "fooLarge").getObjectContent) shouldBe DigestUtils
      .md5Hex(blobs.map(_._1).fold(Array[Byte]())(_ ++ _))
